...
then
	EXPAND_RATE=$8
	/usr/bin/ms $N_SAM $N_REPS -t $THETA -r $RHO $N_SITES -G EXPAND_RATE > $1.txt
elif [ $# = 9 ]
then
	BOTTLE_GEN=$8 
	BOTTLE_SIZE=$9
	/usr/bin/ms $N_SAM $N_REPS -t $THETA -r $RHO $N_SITES -eN $BOTTLE_GEN $BOTTLE_SIZE > $1.txt
fi
...
#Run ngsLD
/usr/bin/ngsLD --verbose 1 --n_ind $N_IND --n_sites $NS --geno $1_reads.testLD.geno --probs --pos $1_pos.txt --max_kb_dist 10 --min_maf 0.05 --rnd_sample 0.05 > $1_reads.testLD.ld
